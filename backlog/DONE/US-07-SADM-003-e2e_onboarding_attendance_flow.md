...
- [x] **Scenario 1: Complete Onboarding Flow**
...
- [x] **Scenario 2: Successful Attendance Flow**
...
- Verified via `backend/tests/e2e_flow_test.py` (API level).
- Verified via `mobile/test/` suite (UI/State level).
- Fixed mobile tests that were failing due to missing mocks and GoRouter context.
- Refactored `AuthNotifier` to allow injecting `FlutterSecureStorage` for testing.
