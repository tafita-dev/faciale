...
class _EnrollScreenState extends ConsumerState<EnrollScreen> {
  ...
  void _submit() async {
    if (_formKey.currentState!.validate()) {
      if (_imagePath == null) {
        ref.read(uxProvider.notifier).showError('please_capture_photo');
        return;
      }

      ref.read(uxProvider.notifier).showLoading('generating_secure_identity');
      ...
      ref.read(uxProvider.notifier).hideLoading();
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen(employeeProvider, (previous, next) {
      if (next.isSuccess) {
        ref.read(uxProvider.notifier).showSuccess('employee_enrolled_successfully');
        ref.read(employeeProvider.notifier).reset();
        context.go('/employees');
      } else if (next.error != null) {
        ref.read(uxProvider.notifier).showError(next.error!);
      }
    });
